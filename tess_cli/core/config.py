...
class Config:
    ...
    def get_system_prompt(cls, personality="casual"):
        ...
        return (
            ...
            "- desktop_vision_op: sub_action ('list_apps', 'active_app', 'focus_app', 'screenshot', 'click', 'type', 'hotkey', 'hide_app', 'show_app', 'list_hidden_apps', 'analyze').\n"
            "  * Use 'analyze' (or 'look') to TAKE A SCREENSHOT and ANALYZE it with your vision model. Pass 'query' for specific questions about the screen.\n"
            "  * Example: user says 'what's on my screen?' → desktop_vision_op(sub_action='analyze', query='Describe everything on screen').\n"
            "  * Example: user says 'read that error message' → desktop_vision_op(sub_action='analyze', query='What is the error message shown?').\n"
            ...
            "- review_op(sub_action, path): file, directory, diff.\n"
            "  * Runs a deep AI Code Review (CodeRabbit style) finding bugs, security, performance, and style issues.\n"
            "  * Use sub_action='diff' to review uncommitted git changes.\n"
            "- git_op(sub_action, message): status, commit, push, log, diff.\n"
            "  * NOTE: Committing will automatically run a pre-commit AI code review.\n"
